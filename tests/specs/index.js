...
describe('ua', function () {
    if (!UA.ie) {
        it('recoginize webkit', function () {
            ...
            expect(ua.webkit).to.equal(537.36);
            expect(ua.safari).to.equal(undefined);
            expect(ua.chrome).to.equal(32.0170041);
        });
        ...
        it('recoginize xiaomi', function () {
            ...
            var ua = UA.getDescriptorFromUserAgent(userAgent);
            expect(ua.webkit).to.equal(534.30);
            expect(ua.safari).to.equal(534.30);
        });
    }
});
